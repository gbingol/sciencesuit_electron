...
import { Worksheet, Range } from '../../lib/ext/grid';
...
window.onload = (evt)=>
{
	get(PAGEID).then(obj=>
	{
		...
		{
			(input as HTMLInputElement).value = obj[input.id];
		}
	});
...
btnCompute.onclick = ((evt)=>
{
	...
	let txtDigits = document.querySelector("txtDigits") as HTMLInputElement;
